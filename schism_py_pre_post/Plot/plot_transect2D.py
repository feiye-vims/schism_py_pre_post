...
def plot_transect_filled(
    ...
    z_i = np.linspace(zs.min(), zs.max(), nz_i)
...
def compare_intrusion_lengths(threshold=0.5, ramp_up_t_records=7*24, reverse=False):
    ...
    """
    results_list = {
        'coarse_mesh': f'/sciclone/schism10/feiye/STOFS3D-v8/O29i1/salinity.transect.{RIVER_NAME}.nc',
        'fine_mesh': f'/sciclone/schism10/feiye/STOFS3D-v8/O19i1/salinity.transect.{RIVER_NAME}.nc',
    }
    intrusion_data_list = []
    for label, filepath in results_list.items():
        ...
        intrusion_length /= 1609.34  # to miles

        ylim = [np.min(intrusion_length) - 10, np.max(intrusion_length) + 10]
        plt.plot(intrusion_length, label=label)
...
def plot_model_transect2D(ax=None, clim=None, draw_mesh=False, reverse_x=False, draw_color_bar=True, river_mile_coor=True):
    """
    ...
    # -------------------- inputs ----------------
    results_ds = xr.open_dataset(f'/sciclone/schism10/feiye/STOFS3D-v8/O29j/salinity.transect.{RIVER_NAME}.nc')
    var_str = 'salinity'  # choose variable to plot
    # --------------------end inupts --------------------
    ...
    if river_mile_coor:
        ...
        plot_transect_filled(miles, -z, var, var_name_dict[var_str], ax=ax, t_idx=-1, cmap='jet', clim=clim, draw_mesh=draw_mesh, draw_color_bar=draw_color_bar)
        ...
        plot_transect_filled(
            river_dist_miles, -z, var, var_name_dict[var_str], ax=ax, t_idx=-1, cmap='jet',
...
if __name__ == "__main__":
    ...
    plot_model_transect2D(river_mile_coor=False, draw_mesh=True)
