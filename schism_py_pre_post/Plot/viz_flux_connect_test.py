...
flux = TimeHistory('/sciclone/schism10/feiye/STOFS3D-v8/R19i1/outputs/flux.out',
...
flux.df.set_index('datetime', inplace=True)
flux.df = flux.df.rolling("6H").mean()
...
plot_groups = {k: groups[k] for k in ['a', 'b', 'c', 'f'] if k in groups}
# plot_groups = {k: groups[k] for k in ['b'] if k in groups}

plt.rcParams.update({'font.size': 18})
fig, axs = plt.subplots(len(plot_groups), 1, figsize=(18, 7.5), sharex=True, constrained_layout=True)
axs = np.atleast_1d(axs)
for i, (group, transects) in enumerate(plot_groups.items()):
    axs[i].plot(
        flux.df.index, flux.df[transects[0]], label=transects[0],
        color='#5fa2d0', linestyle='-', linewidth=4
    )  # downstream

    # for transect in transects[1:]:  # individual upstream transects
    #     axs[i].plot(flux.df.index, flux.df[transect], label=f'Transect {transect}')
    total_flow = np.sum(flux.df[transects[1:]], axis=1)

    axs[i].plot(
        flux.df.index, total_flow, label='Sum of upstream flow',
        color='black', linestyle='--', linewidth=4
    )  # total upstream flow

    percentage = flux.df[transects[0]].mean() / np.mean(total_flow) * 100
    title_str = f'{transects[0]} = {percentage:.2f}% of total upstream flow'

    axs[i].set_title(title_str)
    axs[i].legend()
    axs[i].set_ylabel(r'Flow ($m^3\ s^{-1}$)')
