...
def read_nwm_data(
    ...
    date_range=pd.date_range("2015-06-01 00:00:00", "2015-08-01 00:00:00")
):
    ...
    for this_date in date_range:
        file = glob(f'{data_dir}/*{datetime.strftime(this_date, "%Y%m%d%H%M")}*')
        if len(file) == 0:
    ...
    for i, this_file in enumerate(datafiles):
        ...
        with xr.open_dataset(this_file) as ds:
            ...
            stream_flow[i, :] = ds.stream_flow.data[fids_in_all]
...
def find_usgs_along_nwm(
    ...
    try:
        seg_rec = nwm_shp.loc[starting_seg_id]
        print(f'found the record of the starting segment {starting_seg_id} at {seg_rec} in {nwm_shp}')
    except KeyError:
...
def source_nwm2usgs(
    ...
    # read NWM data
    my_date_range = pd.date_range(start_time_str, end_time_str)
...
if __name__ == "__main__":
    ...
    source_nwm2usgs(
        ...
        output_dir='/sciclone/schism10/feiye/Source_sink/USGS_adjusted_sources/',
    )
